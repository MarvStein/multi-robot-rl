...
def _evaluate_key_presses(
    ...
    wrong_keys = is_pressed & ~active_mask  # (n, TOTAL_KEYS)
    env.wrong_key_pressed[env_ids] = wrong_keys.any(dim=1)

    # Count only rising edges (newly pressed this step) to avoid double-counting held keys
    newly_wrong = wrong_keys & ~prev_is_pressed
    env._total_wrong_keys_pressed[env_ids] += newly_wrong.sum(dim=1).float()
